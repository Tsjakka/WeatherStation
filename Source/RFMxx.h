...
#ifndef USE_SPI_H
    RFMxx(byte mosi, byte miso, byte sck, byte ss, byte irq, byte rst);
#else
    RFMxx(byte ss = SS, byte irq = 17, byte rst = 32); // jeelink en moteino irq 0 / irqPin =2
#endif
    void Init();
    void InitializeForWH1080();
...
#ifndef USE_SPI_H
    byte m_mosi, m_miso, m_sck, m_ss, m_irq, m_rst;
#else
    byte m_ss, m_irq, m_rst;
#endif
    bool m_debug;
